...
function operaciones(op)
{
    ...
    var ops = {
        ...
        raiz: function raizNumeros(n1) {
            const numero = n1;
            const raizCuadrada = Math.pow(numero, 1 / 2);
            console.log(raizCuadrada);
            return (parseInt(raizCuadrada));
        },
        
        potencia: function potenciarNumeros(n1) {
            const num = n1;
            const potenciarNum = Math.pow(num, 1 * 2);
            console.log(potenciarNum);
            return (parseInt(potenciarNum));
    ...
    switch(op) {
        ...
        case 'igual':	
            ...
            switch(memoriaop) {
                ...
                case 'raiz':
                        var operandos = operacion.split("√");
                        var resultado = ops.raiz(operandos[0],operandos[1]);
                        document.getElementById("resultado").value = resultado;
                    break;	
                case 'potencia':
                        var operandos = operacion.split("^");
